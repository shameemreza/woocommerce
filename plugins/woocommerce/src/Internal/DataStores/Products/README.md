...
- `woocommerce_hpps_include_placeholder_in_product_queries`: opt a specific `WP_Query` out of the `product_placeholder` expansion the controller does on `pre_get_posts`. Defaults to true. Useful when an extension hand-rolls a query that must only return rows whose `wp_posts.post_type` is exactly `product`.
...
- **Programmatically-created HPPS products and `WP_Query`.** Products created through `wc_create_product()` or `WC_Product::save()` with no pre-existing post ID land in `wp_posts` with `post_type = 'product_placeholder'` — by design, to avoid firing the legacy `save_post_product` chain on every internal write. To keep these rows visible to every surface that runs `WP_Query( [ 'post_type' => 'product' ] )` directly (REST v3 list endpoints, Store API, ProductCollection block, `[products]` shortcode family, classic widgets, admin Products list), the controller hooks `pre_get_posts` and expands the `post_type` query var to include `product_placeholder` whenever HPPS is on. Use the `woocommerce_hpps_include_placeholder_in_product_queries` filter to opt a specific query out (returns true by default).
