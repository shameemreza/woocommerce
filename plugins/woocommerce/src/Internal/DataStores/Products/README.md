...
- **Programmatically-created HPPS products skip the admin list.** Products created through `wc_create_product()` or `WC_Product::save()` with no pre-existing post ID land in `wp_posts` with `post_type = 'product_placeholder'` — by design, to avoid firing the legacy `save_post_product` chain on every internal write. The admin Products list table queries `post_type = product`, so those rows aren't visible there. The storefront, REST API, and `wc_get_products()` all resolve through HPPS first and find them. Phase 2 will add a query remap so the admin list union covers both post types; for now, an operator can flip a specific row with `wp post update <id> --post_type=product` if they need it visible in wp-admin immediately.
